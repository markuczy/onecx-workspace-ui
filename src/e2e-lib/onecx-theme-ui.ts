...
export class OneCXThemeUiContainer extends OneCXUiContainer {
  constructor(image: string, network: StartedNetwork) {
    ...
    this.withOneCXNameAndAlias('onecx-theme-ui').withOneCXEnvironment({
  ...
  async start(): Promise<StartedOneCXThemeUiContainer> {
    return new StartedOneCXThemeUiContainer(await super.start(), this.getOneCXAppType(), this.getOneCXAlias())
  }
}
