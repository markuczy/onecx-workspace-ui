...
do
  ...
  status_code=`curl --write-out %{http_code} --silent --output /dev/null -X PUT -H 'Content-Type: application/json' "http://localhost:$PORT//operator/slot/v1/$product/$appid" -d @$entry`
