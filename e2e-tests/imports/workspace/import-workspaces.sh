...
do
  ...
  status_code=`curl --write-out %{http_code} --silent --output /dev/null -X POST -H "apm-principal-token: $token" -H 'Content-Type: application/json' "http://localhost:$PORT/exim/v1/workspace/operator" -d @$entry`
  echo "Uploaded workspace $workspace for tenant $tenant with status code $status_code"
done